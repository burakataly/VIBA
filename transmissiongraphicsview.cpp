...
void TransmissionGraphicsView::reset(){ for(auto a:findChildren<QVariantAnimation*>()){a->stop();a->deleteLater();} q.clear(); scene->clear(); addNodes();}
void TransmissionGraphicsView::enqueueEvent(const GuiEvent &e){ q.enqueue({e}); if(q.size()==1) start();}
void TransmissionGraphicsView::start(){ if(!q.isEmpty()) play(q.head().e);}
QColor TransmissionGraphicsView::color(const QString &s) const{
    if(s=="ack"||s=="ack_lost") return "#00bcd4";
    if(s=="corrupt"||s=="ack_bad") return "#f44336";
    return "#8bc34a";
}
void TransmissionGraphicsView::play(const GuiEvent &ev)
{
    ...
    if (!isAck) {
        ...
        hdr->setPen(QPen(Qt::black));   hdr->setBrush(base.lighter(130));
        dat->setPen(QPen(Qt::black));   dat->setBrush(base);
        trl->setPen(QPen(Qt::black));   trl->setBrush(base.lighter(130));
        grp->addToGroup(hdr);
        grp->addToGroup(dat);
        grp->addToGroup(trl);
    } else {
