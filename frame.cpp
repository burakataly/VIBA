...
{
    if (b == 0x7E) { out.append(char(0x7D)).append(char(0x5E)); return; }
    if (b == 0x7D) { out.append(char(0x7D)).append(char(0x5D)); return; }
    out.append(char(b));
}
...
{
    Frame f;
    QByteArray de; de.reserve(raw.size());

    /* FLAG'ler arasını destuff et */
    bool skipFirst = true;
    for (int i = 0; i < raw.size(); ++i) {
        quint8 b = quint8(raw[i]);
        if (b == Frame::FLAG) {           // başlangıç / son
    ...
    if (de.size() != 1 + 13 + 2) return f;   // CTRL+DATA+CRC bekleniyor
    f.ctrl = quint8(de[0]);
    f.data = de.mid(1, 13);
